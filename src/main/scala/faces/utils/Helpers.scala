...
case class Helpers(cfg: FacesSettings)(implicit rnd: Random) {
  ...
  def writeImgPrePost(img: PixelImage[RGBA], id: Int, n: Int, postfix: String): Unit = {
    val outImgPathID = outImgPath + id + "/"
    if (!Path(outImgPathID).exists) {
      Path(outImgPathID).createDirectory(failIfExists = false)
    }
    PixelImageIO.write(img.map { f => f.toRGB }, new File(outImgPathID + id + "_" + n+ "_" + postfix + ".png"))
  }
  ...
  def mooneyfy(imgIn: PixelImage[RGBA], thr: Double = 0.3, blur: Int = 9, blackSwitch: Boolean = true): PixelImage[RGBA] = {
    var cnt = 0;
